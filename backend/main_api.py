...
async def create_comments(comments_data: CommentsCreate, database: Session = Depends(get_db)) -> Comments:
    ...
    db_comments = Comments(
        Name=comments_data.Name,        Comments=comments_data.Comments,        TimeStamp=comments_data.TimeStamp        )
...
async def bulk_create_comments(items: list[CommentsCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_comments = Comments(
                Name=item_data.Name,                Comments=item_data.Comments,                TimeStamp=item_data.TimeStamp            )
            database.add(db_comments)
            database.flush()  # Get ID without committing
...
async def create_legalrequirement(legalrequirement_data: LegalRequirementCreate, database: Session = Depends(get_db)) -> LegalRequirement:
    ...
    db_legalrequirement = LegalRequirement(
        principle=legalrequirement_data.principle,        legal_ref=legalrequirement_data.legal_ref,        standard=legalrequirement_data.standard,
...
async def bulk_create_legalrequirement(items: list[LegalRequirementCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_legalrequirement = LegalRequirement(
                principle=item_data.principle,                legal_ref=item_data.legal_ref,                standard=item_data.standard,
...
async def create_tool(tool_data: ToolCreate, database: Session = Depends(get_db)) -> Tool:
    ...
    db_tool = Tool(
        source=tool_data.source,        version=tool_data.version,        name=tool_data.name,        licensing=tool_data.licensing.value        )
...
async def bulk_create_tool(items: list[ToolCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_tool = Tool(
                source=item_data.source,                version=item_data.version,                name=item_data.name,                licensing=item_data.licensing.value            )
            database.add(db_tool)
            database.flush()  # Get ID without committing
...
async def create_dataset(dataset_data: DatasetCreate, database: Session = Depends(get_db)) -> Dataset:
    ...
    db_dataset = Dataset(
        dataset_type=dataset_data.dataset_type.value,        version=dataset_data.version,        licensing=dataset_data.licensing.value,        source=dataset_data.source,
...
async def bulk_create_dataset(items: list[DatasetCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_dataset = Dataset(
                dataset_type=item_data.dataset_type.value,                version=item_data.version,                licensing=item_data.licensing.value,                source=item_data.source,
...
async def create_observation(observation_data: ObservationCreate, database: Session = Depends(get_db)) -> Observation:
    ...
    db_observation = Observation(
        ...
        observer=observation_data.observer,        whenObserved=observation_data.whenObserved,
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_observation = Observation(
                name=item_data.name,
                description=item_data.description,
                observer=item_data.observer,                whenObserved=item_data.whenObserved,
